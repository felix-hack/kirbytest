...
?>
<div id="<?= $layoutID ?>" class="uk-grid<?= $blockClass ?><?= $subgrid->columnsVerticalAlign() ?><?= $subgrid->columnsHorizontalAlign() ?><?= $subgrid->columnGap() ?><?php e($subgrid->columnsDivider() == "true", ' uk-grid-divider') ?><?= $layoutClass ?><?= $hidden ?><?= $visible ?>" uk-grid<?php if($subgrid->gridMasonry()->isTrue() or $subgrid->gridParallax()->isTrue()): ?>="<?php e($subgrid->gridMasonry()->isTrue(), 'masonry: true;') ?><?php e($subgrid->gridParallax()->isTrue(), 'parallax: 150;') ?>"<?php endif ?><?= $animation ?>>
