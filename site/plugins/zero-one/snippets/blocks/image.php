...
if ($block->location() == 'web' && $block->src()->isNotEmpty()) {
    ...
    $src      = $block->resizeWidth()->isNotEmpty() ? $image->resize($block->resizeWidth()->toInt())->url() : $image->url();
    $srcurl   = $image->url();
    $srcset   = $block->resizeWidth()->isNotEmpty() ? $image->thumb(['width' => $block->resizeWidth()->toInt(), 'format' => 'webp'])->url() : $image->thumb(['format' => 'webp'])->url();
    $width    = $block->resizeWidth()->isNotEmpty() ? $image->resize($block->resizeWidth()->toInt())->width() : $image->width();
    $height   = $block->resizeWidth()->isNotEmpty() ? $image->resize($block->resizeWidth()->toInt())->height() : $image->height();
}
...
<figure<?php if($block->width()->isNotEmpty() or $block->widthMobile()->isNotEmpty() or $block->align()->isNotEmpty() or $block->float()->isNotEmpty()): ?> class="<?= $block->widthMobile() ?><?= $block->width() ?><?= $block->align() ?><?= $block->float() ?>"<?php endif ?><?php if($block->location() == 'kirby' and $block->width()->isEmpty() and $block->widthMobile()->isEmpty()): ?> style="max-width:<?= $width ?>px;"<?php endif ?>>
  ...
  <div uk-lightbox>
      <a href="<?= $srcurl ?>" data-alt="<?= $alt ?>" data-no-swup<?php if ($caption->isNotEmpty()): ?> data-caption="<?= $caption ?>"<?php endif ?>>
        <picture>
          <?php if ($block->location() == 'kirby' && $image = $block->image()->toFile()): ?>
          <source type="image/webp" srcset="<?= $srcset ?>" />
          <?php endif ?>
          <img src="<?= $src ?>"<?= $class ?> alt="<?= $alt ?>"<?php if ($block->location() == 'kirby' && $image = $block->image()->toFile()): ?> width="<?= $width ?>" height="<?= $height ?>"<?php endif ?><?= $ratio ?> loading="lazy">
        </picture>
      </a>
  </div>
