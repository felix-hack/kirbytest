        ...
        <div class="uk-inline-clip uk-transition-toggle<?php e($site->siteBorderRadius() == "true", ' uk-border-rounded') ?>" tabindex="0">
          <?php if($image = $product->gallery()->toFile()): ?>
            ...
            <picture>
              ...
              <img class="uk-transition-scale-up uk-position-cover" src="<?= $image->crop($coverimg_width, $coverimg_height)->url() ?>"<?php e($site->siteBorderRadius() == "true", ' class="uk-border-rounded"') ?> alt="<?= $product->title()->html() ?>" width="<?= $coverimg_width ?>" height="<?= $coverimg_height ?>" loading="lazy">
            </picture>
            <?php endif ?>
          <?php else: ?>
          <picture>
