...
<?php foreach($all_products->pluck('category', ',', true) as $category): ?>
  ...
  <li <?php e($category == $current, ' class="uk-active"') ?>>
    <a href="<?= url($page->url(), ['params' => ['category' => urlencode($category)]]) ?>"<?php e($category == $current, ' class="uk-text-bold"') ?>><?= $category ?></a> <sup>(<?= $all_products->filterBy('category', $category)->count() ?>)</sup>
  </li>
  <?php endforeach ?>
</ul>