...
<ul class="uk-navbar-nav">
  <?php foreach ($items as $item) : ?>
    ...
    <?php elseif ($item->linkType() == "external" && ($item->externalLink()->isNotEmpty() or $item->anchor()->isNotEmpty())) : ?>
    ...
    <div class="uk-navbar-dropdown<?php e($item->megaMenu()->isTrue() && (2 <= $subItems->count()) && ($subItems->count() <= 4), ' uk-navbar-dropdown-width-' . ($item->megaMenuWidth()->isNotEmpty() ? $item->megaMenuWidth() : $subItems->count()), ($item->megaMenu()->isTrue() && $item->megaMenuWidth()->isNotEmpty() ? ' uk-navbar-dropdown-width-' . $item->megaMenuWidth() : null)) ?>" uk-dropdown="animation: uk-animation-slide-bottom-small; duration: 300; offset: 0; animate-out: true; flip: false;<?php e($site->menuPosition() == "left", ' shift: false;') ?>">
          ...
          <div class="<?php e($item->megaMenuBackground()->isTrue(), 'uk-position-relative ') ?>uk-drop-grid uk-child-width-1-<?php e($item->megaMenuItemWidth()->isNotEmpty(), $item->megaMenuItemWidth(), $subItems->count()) ?>" uk-grid>
            ...
            <div>
                <ul class="uk-nav uk-navbar-dropdown-nav">
                  ...
                  <?php elseif ($subItem->linkType() == "external" && ($subItem->externalLink()->isNotEmpty() or $subItem->anchor()->isNotEmpty())) : ?>
                  ...
                  <?php foreach($subSubItems as $subSubItem): ?>
                    ...
                    <?php elseif ($subSubItem->linkType() == "external" && ($subSubItem->externalLink()->isNotEmpty() or $subSubItem->anchor()->isNotEmpty())) : ?>
