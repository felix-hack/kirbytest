<aside>
  ...
  <div class="tm-sidebar-block">
    ...
    <ul class="uk-list uk-list-divider">
      ...
      <li>
        <a href="<?= url($page->url(), ['params' => ['category' => urlencode($category)]]) ?>"><?= Str::ucfirst($category) ?></a> <sup>(<?= $all_articles->filterBy('category', $category)->count() ?>)</sup>
      </li>
      <?php endforeach ?>
    </ul>
  </div>
  ...
  <div class="tm-sidebar-block">
    ...
    <ul class="uk-list uk-list-divider">
      <?php foreach ($all_articles->group(function ($p) { return $p->date()->toDate('Y'); }) as $year => $yearList): ?>
        <li>
          <a href="<?= url($page->url(), ['params' => ['year' => $year]]) ?>"><?= $year ?></a> <sup>(<?= $all_articles->filter(function($p) use($year) { return $p->date()->toDate('Y') == $year;})->count() ?>)</sup>
        </li>
      <?php endforeach ?>
    </ul>
  </div>
  ...
  <div class="tm-sidebar-block">
    ...
    <div>
      ...
      <a href="<?= url($page->url(), ['params' => ['tag' => urlencode($tag)]]) ?>"><span class="uk-label">#<?= $tag ?></span></a>
      <?php endforeach ?>
    </div>
  </div>
  <?php endif ?>
  ...
  <div class="tm-sidebar-block">
    ...
    <a href="<?= $social->url() ?>" target="_blank" class="uk-icon-button uk-button-default" aria-label="<?= $social->platform() ?>" uk-icon="icon: <?= Str::lower($social->platform()) ?>; ratio: 1;"></a>
    <?php endforeach ?>
  </div>
  <?php endif ?>
</aside>